 ...
 ******************************************************************************/
package it.vige.rubia.wildfly.ottozerozero.search;
...
public class ForumsSearchModuleImpl implements ForumsSearchModule {
		...
		if (criteria != null) {
			try {
				...
				if (keywords != null && keywords.length() != 0) {
					...
					MultiFieldQueryParser parser = new MultiFieldQueryParser(LUCENE_36, fields,
	...
	protected void addPostTimeQuery(BooleanQuery query, TimePeriod period) {
		...
		if (period != ALL) {
			...
			if (startDate != null) {
				query.add(new TermRangeQuery("createDate", dateToString(startDate, MINUTE),
						dateToString(endDate, MINUTE), true, true), MUST);
			}
		}
	}
